...
export const putDb = async (content) => {
  ...
  const userDb = await openDB(DB_NAME, 1);

  // TODO: Now create a variable for the transaction; again, this will be referenced below.
  const dbTransacation = userDb.transaction(DB_NAME, 'readwrite');

  // TODO: Now create a variable for the store
  const dbStore = dbTransacation.objectStore(DB_NAME);
...
export const getDb = async () => {
  ...
  const userDb = await openDB(DB_NAME, 1);
  const dbTransacation = userDb.transaction(DB_NAME, 'readonly');
  const dbStore = dbTransacation.objectStore(DB_NAME);
